...
public abstract class OperatorBase implements java.io.Serializable {
    ...
    /**
     * 执行指令 （所有Operator的指令都会执行这个公共的方法，里面会跳转到具体的 Operator实现类）
     * @param context
     * @param list
     * @param errorList
     * @return
    ...
    public OperateData execute(InstructionSetContext context, ArraySwap list, List<String> errorList) throws Exception {
        //会 先执行Operator.executeInner(), 然后执行具体实现类的 executeInner()
        OperateData result = executeInner(context, list);

        //输出错误信息
        if (errorList != null && errorInfo != null && result != null) {
            Object obj = result.getObject(context);

            if (obj != null && obj instanceof Boolean && ((Boolean) obj).booleanValue() == false) {
                String tmpStr = ExpressUtil.replaceString(errorInfo, toObjectList(context, list));
    ...
    public String[] getOperDataAnnotation() {
